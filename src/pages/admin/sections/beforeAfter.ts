...
function draw(root: HTMLElement, projects: BeforeAfter[]): void {
  const title = h("input", { class: "input", placeholder: "Project title" }) as HTMLInputElement;
  const before = h("input", { class: "input", placeholder: "Before image URL" }) as HTMLInputElement;
  const after = h("input", { class: "input", placeholder: "After image URL" }) as HTMLInputElement;
  ...
  root.replaceChildren(
    h("h2", { class: "admin-section__title" }, ["Before / After"]),
